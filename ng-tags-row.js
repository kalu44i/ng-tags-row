...
'use strict';
    ...
    var tagsModule = angular.module('ngTagsRow', []);
     ...
     * property "tooltip" with the value to show. If
     *
     */
    tagsModule.directive('tagsViewer', function () {
        return {
            restrict: 'E',
            require: 'ngModel',
            scope: {
            ...
            templateUrl: '/DataManager/angular/app/templates/analytics/tags-viewer.html',
            replace: false,
            transclude: true,
            link: function (scope) {
                ...
                scope.calculateTags = function () {
                    ...
                    setTimeout(function () {
                        var mainBlockWidth = angular.element(".mainContainer").innerWidth();
                        var dropdownMenuBlockWidth = angular.element(".dropdownContainer").outerWidth(true);
                        var sum = 0;
                        var tagsBlockWidth = mainBlockWidth - dropdownMenuBlockWidth;
                ...
                scope.$removeTag = function (tag, index, isMainTag) {
                    ...
                    if (isMainTag) {
                        scope.tags = scope.tags.splice(index, 1);
                    } else {
                        scope.tags = scope.tags.splice(index + scope.lastMainIndex + 1, 1);
                    }
